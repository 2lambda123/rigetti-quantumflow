...
def test_diamond_norm():
    ...
    assert np.isclose(2.0, dn, rtol=TOL, atol=TOL)
    ...
    for turns, target in turns_dnorm:
        ...
        dn = qf.diamond_norm(chan0, chan1)
        assert np.isclose(target, dn, rtol=TOL, atol=TOL)
    ...
    for p, target in hadamard_mixtures:
        ...
        dn = qf.diamond_norm(chan0, chan1)
        assert np.isclose(dn, target, rtol=TOL, atol=TOL)

    chan0 = qf.TY(0.5, 0).aschannel()
    chan1 = qf.I(0).aschannel()
    dn = qf.diamond_norm(chan0, chan1)
    assert np.isclose(dn, np.sqrt(2), rtol=TOL, atol=TOL)
